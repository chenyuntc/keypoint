...
def load_annotations(anno_file, return_dict):
    ...
    try:
        ...
        return_dict[
            'error'] = 'Annotation file does not exist or is an invalid JSON file.'
...
def load_predictions(prediction_file, return_dict):
    ...
    try:
        ...
        return_dict[
            'error'] = 'Prediction file does not exist or is an invalid JSON file.'
        exit(return_dict['error'])
    ...
    for i in range(anno_count):
        ...
        if np.sum(visible) == 0:
            ...
            for j in range(predict_count):
                ...
                # print ("dis: ", dis)
                # print ("in exp:", np.exp(-dis / 2 / delta[visible]**2 / (scale + 1)))
                oks[i, j] = np.mean(
...
def main(**kwargs):
    ...
    annotations = load_annotations(anno_file=opt.ref, return_dict=return_dict)
    print ('Complete reading annotation JSON file in %.2f seconds.' % (
        time.time() - start_time))

    # Load prediction JSON file
    start_time = time.time()
    predictions = load_predictions(
        prediction_file=opt.submit, return_dict=return_dict)
    print ('Complete reading prediction JSON file in %.2f seconds.' % (
        time.time() - start_time))
