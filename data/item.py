...
class Item(object):
    ...
    def update_anno(self):
        ...
        for human,human_anno in self.anno['human_annotations'].items():
            human_anno[1],human_anno[0], human_anno[3],human_anno[2] =             human_anno
    ...
    def confidence_map(self):
        ...
        for part in range(14):
            maps = []
            for human,middle in self.anno['keypoint_annotations'].items():
                if middle[part][2]==3:
                    # 不在图中的初始化为零
                    map = np.zeros([_h,_w])
                else:
                ...
                maps.append(map)
            part_map = np.stack(maps, -1).max(axis=2)
    ...
    def _PAF(self,center1,center2,ratio):
        ...
        limb_vec_unit = (center2-center1)/(length+0.00001)
        limb_perp_unit = np.array([-limb_vec_unit[1],limb_vec_unit[0]])
        width = int(length*ratio/2)+2
    ...
    def get(self):
        ...
        ratio = min(ratio)
        re_size = (size/ratio).astype(np.int32)

        img = self.resize(img,re_size.tolist())
        paf = self.resize(paf.reshape(paf.shape[0],-1,13*2),re_size.tolist())
        cf = self.resize(cf,re_size.tolist())
        mask = self.resize(mask,re_size.tolist())

        crop_width = re_size - output_size
